...
object Z80 {
  ...
  trait EventListener {
    ...
    def interrupted(z80:Z80,mode:Int,isNMI:Boolean): Unit
    def reset(z80:Z80): Unit
    def halt(z80:Z80,isHalted:Boolean): Unit
  }
  ...
  class Context(val mem:Memory,val io:IOMemory) {
    ...
    |---|---|---|---|---|---|---|---|
    */
    inline val CFLAG = 0x1
    inline val NFLAG = 0x2
    inline val PFLAG = 0x4
    inline val XFLAG = 0x8
    inline val HFLAG = 0x10
    inline val YFLAG = 0x20
    inline val ZFLAG = 0x40
    inline val SFLAG = 0x80
    ...
    final def writeW(address:Int,value:Int): Unit = {
      ...
      mem.write((address + 1) & 0xFFFF,(value >> 8) & 0xFF)
    }
  ...
  private case class Opcode(opcodes:Array[Int],
    ...
    val opcode : Int = opcodes.foldLeft(0)((a:Int,i:Int) => a << 8 | i)
...
class Z80(_mem:Memory,
  ...
  private val mem = new Memory {
    ...
    override final def write(address: Int, value: Int): Unit = {
      if (notifyEventListeners) onRWEvent(address, isRead = false,value)
      _mem.write(address,value)
    }
  }

  val ctx = new Context(mem,io_memory)
  inline val M1FETCH_PIN = 1
  inline val REFRESH_PIN = 2
  inline val DUMMY_READ_PIN = 4
  ...
  // ======================================== Bus Request (three state) ======================================
  def requestBUS(request:Boolean): Unit = busREQ = request
  def isBUSRequested : Boolean = busREQ
  // ======================================== Fetch & Execute ================================================
  ...
  final def clock() : Int = {
    ...
    if ((irqLow || nmiOnNegativeEdge) && !ctx.mustDelayInt) { // any interrupt pending ?
      ...
      if (nmiOnNegativeEdge) { // NMI
        if (ctx.halted) {
          ctx.halted = false
          ctx.incPC()
          if (notifyEventListeners) onHaltEvent(halted = false)
        }
        ...
        ctx.IFF1 = 0
        ctx.PC = 0x0066
        if (notifyEventListeners) onInterruptEvent(0,isNMI = true)
        return 11
      }
      else { // IRQ
        if (ctx.IFF1 == 1) {
          ...
          ctx.im match {
            case 0 =>
              ...
              if (notifyEventListeners) onInterruptEvent(0,isNMI = false)
              return 12
            case 1 =>
              ctx.PC = 0x38
              if (notifyEventListeners) onInterruptEvent(1,isNMI = false)
              return 13
            case 2 =>
              interruptMode2Handling()
              if (notifyEventListeners) onInterruptEvent(2,isNMI = false)
              return 19
          }
          return 0
        }
    ...
    if (notifyEventListeners) onFetchEvent(pcFetch,opcode.opcode)
    // execute
    lastPC = ctx.PC
    ctx.copyQ
    opcode.executeFunction(ctx)
