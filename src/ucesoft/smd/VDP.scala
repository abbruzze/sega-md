...
class VDP extends SMDComponent with Clock.Clockable with M6800X0.InterruptAckListener:
  ...
  private var statusRegister = STATUS_FIFO_EMPTY_MASK
  ...
  final def readHVCounter: Int = (vcounter & 0xFF) << 8 | (hcounter << 1) & 0xFF
  /*
    Register set
  ...
  inline private def writeRegister(reg:Int,value:Int): Unit =
    ...
    reg match
      case 12 => // REG #12 |RS0 0 0 0 S/TE LSM1 LSM0 RS1|
        val mode = if REG_H32 then HMode.H32 else HMode.H40
        if hmode != mode then
          hmode = mode
          changeVDPClockDivider(hmode.initialClockDiv)
